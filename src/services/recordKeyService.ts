...
const SECRET_SALT = "THANH_DONG_YEN_0915213717_2026_RECORD_KEY";
const TRIAL_SEC_SALT = "DVT_ANTI_TAMPER_TRIAL_PROTECT_2026_RECORD";
const STORAGE_SEC_TRIAL = "gvai_record_sec_trials_v1";
const STORAGE_BACKUP_HASH = "_sys_hw_rec_hash";
...
export function getOrCreateRecordHardwareCode(): string {
  if (typeof window === 'undefined') return 'DVT-REC-DEFAULT';

  const STORAGE_KEY = 'gvai_record_hardware_code';
  let code = localStorage.getItem(STORAGE_KEY);
  if (code && code.startsWith('DVT-REC-')) {
    return code;
  }
  ...
  const rawSeed = `REC-${screenPart}-${corePart}-${navigator.userAgent}`;
 ...
 */
export async function getSecureRecordTrialRemaining(machineId: string): Promise<number> {
 ...
 */
export async function consumeSecureRecordTrial(machineId: string): Promise<number> {
  const current = await getSecureRecordTrialRemaining(machineId);
...
export async function generateRecordLicenseKey(
  ...
  const key = `REC-${prefix}-${expHex}-${sig}`;
 ...
 */
export async function verifyRecordLicenseKey_orig(key: string, machineId: string): Promise<RecordVerifyResult> {
  const cleanKey = key.trim().toUpperCase();
  const cleanId = machineId.trim().toUpperCase();

  const parts = cleanKey.split('-');
  if (parts.length !== 4 || parts[0] !== 'REC') {
    return {
      isValid: false,
      message: 'Mã kích hoạt không đúng định dạng (Ví dụ: REC-LT-...)!'
    };
  }
...
export async function verifyRecordLicenseKey(...args: any[]): Promise<any> {
  ...
  return (verifyRecordLicenseKey_orig as any)(...args);
}
