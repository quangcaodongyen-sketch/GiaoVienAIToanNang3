...
const SECRET_SALT = "DVT_PDF_SUITE_VIP_SECRET_2026_0915213717";
const TRIAL_SEC_SALT = "DVT_ANTI_TAMPER_TRIAL_PDF_2026";
const STORAGE_SEC_TRIAL = "gvai_pdf_sec_trials_v1";
const STORAGE_BACKUP_HASH = "_sys_hw_pdf_hash";
const STORAGE_VIP_LICENSE = "gvai_pdf_vip_license";
 ...
 * Định dạng: DVT-PDF-XXXX-XXXX
 */
export function getOrCreatePDFHardwareCode(): string {
  if (typeof window === 'undefined') return 'DVT-PDF-7788-B2C3';

  const STORAGE_KEY = 'gvai_pdf_hardware_code';
  let code = localStorage.getItem(STORAGE_KEY);
  if (code && code.startsWith('DVT-PDF-') && code.length >= 17) {
    return code;
  }
  ...
  const rawSeed = `PDFSUITE-${screenPart}-${corePart}-${navigator.userAgent}`;
  ...
  const hex2 = Math.abs((hash * 53) | 0).toString(16).toUpperCase().padStart(4, '0').slice(-4);
  code = `DVT-PDF-${hex1}-${hex2}`;
 ...
 */
export async function getSecurePDFTrialRemaining(): Promise<number> {
  if (typeof window === 'undefined') return 5;

  if (isPDFVIPActivated()) {
    return 999;
  }

  const hwid = getOrCreatePDFHardwareCode();
 ...
 */
export async function consumeSecurePDFTrial(): Promise<number> {
  if (typeof window === 'undefined') return 0;
  if (isPDFVIPActivated()) return 999;

  const current = await getSecurePDFTrialRemaining();
 ...
 */
export function isPDFVIPActivated(): boolean {
  if (typeof window === 'undefined') return false;
  const key = localStorage.getItem(STORAGE_VIP_LICENSE);
  return Boolean(key && key.startsWith('PDF-'));
}
...
export async function verifyPDFLicenseKey(
  ...
  const expectedLifetime = await generatePDFLicenseKey(cleanHwid, 'lifetime');
  ...
  const expected2Year = await generatePDFLicenseKey(cleanHwid, '2year');
  ...
  const expected1Year = await generatePDFLicenseKey(cleanHwid, '1year');
 ...
 */
export async function activatePDFLicense(
  key: string,
  machineId: string
): Promise<PDFVerifyResult> {
  const result = await verifyPDFLicenseKey(key, machineId);
  if (result.isValid) {
    localStorage.setItem(STORAGE_VIP_LICENSE, key.trim().toUpperCase());
    localStorage.setItem('gvai_pdf_license_pkg', result.packageName || 'VIP');
    localStorage.setItem('gvai_pdf_license_date', new Date().toISOString());
  }
  return result;
}
 ...
 */
export function buildPDFZaloMessage(
  machineId: string,
  licenseKey: string,
  pkgName: string
): string {
...
2. Mở cửa sổ "PDF Suite Pro" -> Chọn Tab "Bản Quyền & Kích Hoạt".
