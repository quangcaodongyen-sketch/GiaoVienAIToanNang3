  ...
  public async submitRegistrationToCloud(req: {
    ...
    try {
      ...
      const payloadData: RegistrationRequest = {
        ...
        price: req.price || (req.packageType === '1YEAR' ? '200.000đ' : req.packageType === '2YEAR' ? '250.000đ' : 'Miễn phí'),
        status: 'PENDING',
        createdAt: now
      };
...
- **Gói đăng ký:** **${req.packageType === '1YEAR' ? '1 Năm (200.000đ)' : req.packageType === '2YEAR' ? '2 Năm (250.000đ)' : 'Dùng thử 5 lần'}**
- **Thời gian gửi:** ${now}
...
`;
      ...
      const response = await fetch(`${GITHUB_API_URL}/issues`, {
        ...
        headers: getHeaders(),
        body: JSON.stringify({
          title: `[ĐĂNG KÝ CLOUD] ${req.machineId} - ${req.fullName} - ${req.packageType === '1YEAR' ? '1 Năm (200k)' : req.packageType === '2YEAR' ? '2 Năm (250k)' : 'Dùng thử'}`,
          body: bodyText,
      ...
      for (const issue of issues) {
        ...
        if (!regData) {
          regData = {
            ...
            price: issue.title.includes('2YEAR') ? '250.000đ' : '200.000đ',
