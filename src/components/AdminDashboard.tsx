  ...
  const handleGenerateCHVBKey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (chvbPackage === '1year') {
        pkgName = 'GÓI BẢN QUYỀN 1 NĂM (200.000đ)';
        expDateStr = '1 Năm';
      } else if (chvbPackage === '2year') {
        pkgName = 'GÓI BẢN QUYỀN 2 NĂM (250.000đ)';
        expDateStr = '2 Năm';
      }
              ...
              <div className="flex-1 flex flex-col min-h-0 space-y-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  ...
                  <p className="text-[11px] text-slate-400">
                    💡 Bấm duyệt 1-click để hệ thống tự động kích hoạt VIP (1 năm 200k / 2 năm 250k) và lưu người duyệt.
                  </p>
                </div>
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          await licenseService.createDirect({
                                            ...
                                            notes: `Kích hoạt nhanh 1 Năm (200k) bởi ${currentAdminName}`
                                          }, currentAdminName);
                                          await loadData();
                                          loadTrackingData();
                                          alert(`Đã kích hoạt Gói 1 Năm (200k) cho máy ${item.machineId}!`);
                                        }}
                                        className="px-2 py-1 rounded bg-sky-600 hover:bg-sky-500 text-white font-bold text-[10px] transition cursor-pointer"
                                        title="Kích hoạt nhanh 1 Năm (200k)"
                                      >
                                        + 1 Năm (200k)
                                      </button>
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          await licenseService.createDirect({
                                            ...
                                            notes: `Kích hoạt nhanh 2 Năm (250k) bởi ${currentAdminName}`
                                          }, currentAdminName);
                                          await loadData();
                                          loadTrackingData();
                                          alert(`Đã kích hoạt Gói 2 Năm (250k) cho máy ${item.machineId}!`);
                                        }}
                                        className="px-2 py-1 rounded bg-amber-600 hover:bg-amber-500 text-white font-bold text-[10px] transition cursor-pointer"
                                        title="Kích hoạt nhanh 2 Năm (250k)"
                                      >
                                        + 2 Năm (250k)
                                      </button>
                                    </>
              ...
              <form onSubmit={handleGenerateCHVBKey} className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      <option value="1year">Gói 1 Năm (200.000đ)</option>
                      <option value="2year">Gói 2 Năm (250.000đ)</option>
